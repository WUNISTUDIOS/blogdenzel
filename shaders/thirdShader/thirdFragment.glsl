uniform float uTime;
varying vec2 vUv;
vec3 colorA = vec3(1.0, 0.0, 0.0);
vec3 colorB = vec3(0.0, 1.0, 0.0);
vec3 colorC = vec3(1.0, 1.0, 1.0);

// circle sdf
float sdCricle(in vec2 p, in float r) {
        return length(p) - r;
}

  vec2 rotate(vec2 p, float a) {
    float s = sin(a);
    float c = cos(a);
    return vec2(p.x * c + p.y * s, -p.x * s + p.y * c);
  }

void main() {
        // distorting uv space into star
        vec2 uvRotation = rotate(vUv - 0.5, uTime * 0.05);
        float sigTime = sin(uTime * 0.5) * 0.1;
        // float distance = fract(sdCricle(vUv - vec2(0.5), 0.5)) * sigTime;
        float distance = fract(sdCricle(uvRotation, 1.5)) * sigTime;
        distance *= 0.01 / sdCricle(uvRotation.x - 0.5 * vec2(0.5), 0.1);
        distance *= 0.1 / sdCricle(uvRotation.y - 0.5 * vec2(0.5), 0.5);
        
        // coloring
        vec3 col = (distance > 0.0) ? colorA + 0.2 : colorC + 0.2;
        col = col * exp(distance);
        col *= 1.0 - exp(-6.0 * abs(distance));
        col *= 0.9 + 0.2 * cos(20.0 * distance) * vec3(sin(uTime), sin(uTime + 2.0),sin(uTime + 4.0));
        gl_FragColor = vec4(col, 1.0);
}
